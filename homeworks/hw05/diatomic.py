...
class CO_molecule:
    ...
    def v_K(self):  # return kinetic energy of the vibration motion
        K_C = 0.5 * m_C * mag2(proj(self.vrel_C(), self.bond.axis))
        K_O = 0.5 * m_O * mag2(proj(self.vrel_O(), self.bond.axis))
        return K_C + K_O

    def r_K(self):  # return kinetic energy of the rotational motion
        K_C = 0.5 * m_C * mag2(self.vrel_C() - proj(self.vrel_C(), self.bond.axis))
        K_O = 0.5 * m_O * mag2(self.vrel_O() - proj(self.vrel_O(), self.bond.axis))
        return K_C + K_O
