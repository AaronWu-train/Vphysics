...
# 創建控制元件
displacement_slider = slider(min=0, max=10, value=3, length=300, bind=None, label="初始位移")
damping_slider = slider(min=0, max=0.2, value=0.05, length=300, bind=None, label="阻尼常數")

# 變數儲存初始設定
damping = damping_slider.value
initial_displacement = displacement_slider.value

# 定義開始模擬函數
def start_simulation(evt):
    global damping, initial_displacement
    # 更新阻尼因子與初始位移
    damping = damping_slider.value
    initial_displacement = displacement_slider.value
    print(f"開始模擬: 阻尼={damping}, 初始位移={initial_displacement}")
    ...
    # 添加初始位移作為波動源
    balls[n//2].pos.y += initial_displacement # 中間的球向上位移

    # 模擬主循環
    t = 0
    natural_length = length / (n - 1)  # 每段彈簧的自然長度
    while t < 10:  # 限制模擬時間，避免無限循環
        rate(1000)  # 控制模擬速度
...
# 創建開始模擬按鈕並綁定事件
start_button = button(text="開始模擬", pos=scene.title_anchor, bind=start_simulation)
